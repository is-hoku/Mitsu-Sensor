...
while True:
    ...
    frame1 = frame1[26:, 16:]
    frame2 = frame2[:337, :626]
    frame1 = cv2.copyMakeBorder(frame1, 13, 13, 8, 8, cv2.BORDER_CONSTANT, 0)
    frame2 = cv2.copyMakeBorder(frame2, 12, 11, 7, 7, cv2.BORDER_CONSTANT, 0)
    ...
    for(x, y, w, h) in human1:
        ...
        # cv2.circle(frame1, (x+w//2, y+h//2), 15, (0, 50, 255), thickness=-1)
    ...
    frame11 = cv2.GaussianBlur(cv2.equalizeHist(frame11),(7,7), 6)
    frame22 = cv2.GaussianBlur(cv2.equalizeHist(frame22),(7,7), 6)
    # out = stereo.compute(frame1, frame2).astype(np.float32) / 16
